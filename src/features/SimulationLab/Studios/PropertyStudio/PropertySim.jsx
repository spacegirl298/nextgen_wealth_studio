...
– Sliders update the comparison table and verdict in real-time.
*/
...
/* ── Pure calculation function (no setState) ─────────────────── */
function calculate({ horizon, purchasePrice, bondRate, bondTerm, savings, priceGrowth, monthlyRent, rentalInflation }) {
  const years = horizon;
...
export default function PropertyLab() {
  ...
  /* Live results — recalculates whenever any input changes */
  const results = useMemo(
    () => calculate({ horizon, purchasePrice, bondRate, bondTerm, savings, priceGrowth, monthlyRent, rentalInflation }),
    [horizon, purchasePrice, bondRate, bondTerm, savings, priceGrowth, monthlyRent, rentalInflation],
  );
    ...
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        ...
        <p className={styles.heroSub}>
          Adjust the sliders below to compare the financial outcomes of renting versus buying in Johannesburg. Results update in real-time.
        </p>
      </div>
        ...
        {learnOpen && (
          <div className={styles.learnBody}>
            <p>
              ...
              bond, levy, security upgrade, and municipal rate issue. Renting offers flexibility, predictable monthly
              costs, and no maintenance costs, but your monthly payment builds no equity. In Johannesburg specifically,
              area decline can happen quickly due to crime, potholes, or businesses moving, so renters can leave easily
      ...
      <div className={styles.sectionCard}>
        ...
        <SliderField label="Gross Monthly Income"            min={5000}   max={200000} step={1000}  value={monthlyIncome}  onChange={setMonthlyIncome}  prefix="R " info />
        <SliderField label="Available Monthly Housing Budget" min={2000}   max={60000}  step={500}   value={housingBudget}  onChange={setHousingBudget}  prefix="R " info />
        <SliderField label="Savings Available for Deposit"   min={0}      max={2000000} step={10000} value={savings}        onChange={setSavings}        prefix="R " info />
        <SliderField label="Time Horizon"                    min={1}      max={30}     step={1}     value={horizon}        onChange={setHorizon}        suffix=" years" info />
      </div>
      ...
      <div className={styles.twoCol}>
        <div className={styles.sectionCard}>
          ...
          <p className={styles.cardSub}>
            Buying includes bond repayment, property rates, maintenance and opportunity cost of deposit. You hold equity but have less liquidity.
          </p>
          <SliderField label="Purchase Price"              min={300000} max={5000000} step={50000} value={purchasePrice} onChange={setPurchasePrice} prefix="R " info />
          <SliderField label="Bond Interest Rate"          min={7}      max={20}      step={0.1}   value={bondRate}      onChange={setBondRate}      suffix="%" info />
          <SliderField label="Bond Term"                   min={5}      max={30}      step={1}     value={bondTerm}      onChange={setBondTerm}      suffix=" years" info />
          <SliderField label="Annual Property Price Growth" min={0}      max={15}      step={0.5}   value={priceGrowth}   onChange={setPriceGrowth}   suffix="%" info />
        </div>

        <div className={styles.sectionCard}>
          <h2 className={styles.cardTitle}>Renting</h2>
          <p className={styles.cardSub}>
            Renting has no maintenance costs or bond risks, but you don't build equity over time.
          </p>
          <SliderField label="Monthly Rent"           min={2000} max={40000} step={250} value={monthlyRent}      onChange={setMonthlyRent}      prefix="R " info />
          <SliderField label="Annual Rental Inflation" min={0}    max={15}    step={0.5} value={rentalInflation}  onChange={setRentalInflation}  suffix="%" info />
        </div>
      </div>
      ...
      <div className={styles.resultsGrid}>
        <div className={styles.sectionCard}>
          <h2 className={styles.cardTitle} style={{ marginBottom: "1rem" }}>
            Buying vs Renting over {horizon} {horizon === 1 ? "year" : "years"}
          </h2>
          <table className={styles.table}>
            ...
            <tbody>
              {TABLE_ROWS.map(([label, rent, buy, prefix]) => (
                <tr key={label}>
